...
if (isset($_POST['current-password'])) {
    ...
    if ($new == $confirm) {
        ...
        if ($res->rowCount() > 0) {
            ...
            $sql1 = "UPDATE `student` SET `password`='$new' WHERE `email` = '$email' ";
            $res = $conn->query($sql1);
        } else {
                ...
                <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
                    ...
                    <div class="password-input-container">
                        <input type="password" id="current-password" name="current-password"
                            style="margin-bottom: 20px !important" class="form-control" name="password" required>
                    ...
                    <div class="password-input-container">
                        <input type="password" id="new-password" name="new-password"
                            style="margin-bottom: 20px !important" class="form-control" name="password"
                            pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
