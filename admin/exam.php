...
if (isset($_GET['course_name'])) {
    ...
    $time = date('h:i A', strtotime($_GET['time']));
    $date = $_GET['date'];




    $select = ("SELECT * FROM exam WHERE
    ...
    if ($count == 0) {
        ...
        $sql = 'INSERT INTO `exam`(`course_name`, `course_date`, `course_time`) VALUES (?,?,?)';

        $stmt = $conn->prepare($sql);

        $execution = $stmt->execute([$name, $time, $date]);
        header('location: exam.php');
    } else {

        $error1 = "this date already  exists";
    }
}
?>
...
<body>
    ...
    <nav id="sidebar" class="js-fullheight">
        ...
        <div class="p-4">
            ...
            <ul class="list-unstyled listItem px-3">
                ...
                <li class=""><a href="courses.php" class="text-decoration-none px-3 py-2 d-block">
                    ...
                    Courses</a></li>
                    <hr class="mb-0 mt-0 bg-white" >
                <li class="active"><a href="exam.php" class="text-decoration-none px-3 py-2 d-block">
        ...
        <div class="home">
            <div class="container">
                <h1 class="display-6"><i class="fa fa-calendar-days text-primary"></i> Exam <span
                        class="text-primary">time</span></h1>
                <nav class="navbar bg-body-tertiary navbar-light justify-content-center mt-4">
                    ...
                    <form class="d-flex" role="search">
                        <input id="search" class="form-control mr-sm-2" type="search"
                            placeholder="Search for Exam time..." aria-label="Search">&nbsp;
                        ...
                        <form id="book-form" name="form1" class="mt-4" action="<?php $_SERVER['PHP_SELF'] ?>"
                            ...
                            <div class="form-group">
                                ...
                                <input required pattern="[a-zA-Z\s]+" title="Please enter alphabets and spaces only"
                                    type="text" name="course_name" id="author" class="form-control" onkeypress="return (event.charCode > 64 && 
                  event.charCode < 91) || (event.charCode > 96 && event.charCode < 123)">
                            </div>
                            <div class="form-group">
                                <label for="link">Day</label>
                                <input type="date" name="date" id="link" class="form-control" required>
                            </div>

                            <div class="form-group">
                                <label for="link">Time</label>
                                <input type="time" name="time" id="link" class="form-control" required>
                            </div>
                            <input type="submit" name="submit" value="Add Exam Time" class="button btn-block">
                        </form>
                    </div>
                </div>
                <br>
                    ...
                    <table class="table table-striped text-center" style="width: 100%">
                        ...
                        <tbody id="myTable">
                            ...
                            <tr>
                                ...
                                <?php echo '<td>' . $row['course_time'] .  '</td>' ?>
                                <?php echo '<td>' . $row['course_date'] .  '</td>' ?>
                                <td>
                                    <a href="delete.php?course_name=<?php echo $row['course_name'] ?>&type=exam
